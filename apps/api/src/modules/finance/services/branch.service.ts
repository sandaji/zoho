...
export class BranchService {
  ...
  private async getBranchKPIs(
    ...
    try {
      ...
      const glKPIs = await analytics.getBranchKPIs(
        branchId,
        monthStart,
        monthEnd,
      );
      ...
      const totalInventoryValue = inventoryData.reduce(
        (sum: number, inv: any) =>
          sum + (inv.product?.unit_price ?? 0) * (inv.quantity ?? 0),
        0,
      );
      ...
      return {
        ...
        profit_margin:
          glKPIs.monthRevenue > 0
            ? (glKPIs.monthProfit / glKPIs.monthRevenue) * 100
            : 0,
  ...
  private calculateAverageDeliveryTime(deliveries: any[]): number {
    ...
    const totalTime = completedDeliveries.reduce((sum: number, d: any) => {
      ...
      const deliveredTime = new Date(d.scheduled_date || new Date()).getTime();
