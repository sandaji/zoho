...
import { ROLE_BLUEPRINTS } from "@config/rbac-config";
...
export class RbacManagementService {
  ...
  static async syncRoleWithBlueprint(
    ...
    blueprint.packs.forEach((pack) =>
      pack.forEach((code) => allPermissionCodes.add(code)),
    );
    ...
    try {
      return await prisma.$transaction(async (tx) => {
        ...
        if (!role) {
          throw new Error(
            `Cannot sync: Role with code "${roleCode}" not found in database.`,
          );
        }
      ...
      logger.error(
        { roleCode, blueprintKey, error },
        "❌ Failed to sync role with blueprint",
      );
      throw error;
    }
  }
  ...
  static async bootstrapSystemRoles() {
    ...
    const rolesToSync: Array<{
      code: string;
      blueprint: keyof typeof ROLE_BLUEPRINTS;
    }> = [
