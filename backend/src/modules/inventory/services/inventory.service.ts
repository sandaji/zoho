...
import { notFoundError, AppError, ErrorCode } from "@core/errors/errors";
...
export class InventoryService {
  ...
  static async logTransferAudit(
    ...
    try {
      ...
      logger.error(
        { err, transferId, action },
        "Failed to write transfer audit log entry",
      );
    }
  }
  ...
  static async depleteStockFIFO(
    ...
    const { totalCost, batchesUsed } =
      await InventoryService.depleteBatchesOnly(
        tx,
        productId,
        warehouseId,
        requestedQty,
      ...
      async (tx) => {
        ...
        if (adjustmentType === "increase") {
          ...
          const unitCost = new Prisma.Decimal(
            dto.unitCost ?? product.cost_price ?? 0,
          );
  ...
  async transferInventory(
    ...
    return this.prisma.$transaction(
      async (tx) => {
        ...
        const unitCost =
          quantity > 0 ? totalCost.div(quantity) : new Prisma.Decimal(0);
  ...
  async requestTransfer(userId: string, dto: RequestTransferDTO): Promise<any> {
    ...
    return await this.prisma.$transaction(
      async (tx) => {
        ...
        await InventoryService.logTransferAudit(
          tx,
          transfer.id,
          "CREATE",
          userId,
    ...
    return this.prisma.$transaction(
      async (tx) => {
        for (const item of dto.items) {
          ...
          if (
            item.picked_qty < 0 ||
            item.picked_qty > originalItem.requested_qty
          ) {
            throw new AppError(
              ErrorCode.VALIDATION_ERROR,
              400,
        ...
        logger.info(
          { transferId: updated.id, userId },
          "Stock transfer picking completed, awaiting verification",
        );
    ...
    return await this.prisma.$transaction(
      async (tx) => {
        ...
        for (const item of dto.items) {
          ...
          const variance =
            originalItem.dispatched_qty -
            (item.received_qty + item.damaged_qty);
  ...
  async getTransferAnalytics(): Promise<any> {
    ...
    for (const t of completedTransfers) {
      if (t.receivedAt && t.createdAt) {
        totalCycleTimeMs +=
          new Date(t.receivedAt).getTime() - new Date(t.createdAt).getTime();
      }
    }
    ...
    const avgCycleTimeHours =
      completedTransfers.length > 0
        ? Number(
            (
              totalCycleTimeMs /
              (completedTransfers.length * 1000 * 60 * 60)
            ).toFixed(1),
          )
        : 0;
    ...
    const discrepancyRatePercent =
      totalTransfers > 0
        ? Number(
            ((discrepancyTransfersCount / totalTransfers) * 100).toFixed(1),
          )
        : 0;

    const truckCount = transfers.filter(
      (t) => t.dispatchMode === "TRUCK",
    ).length;
    const riderCount = transfers.filter(
      (t) => t.dispatchMode === "RIDER",
    ).length;
    ...
    const pendingApprovals = transfers.filter(
