...
export class AccountingService {
  ...
  static async getBalanceSheet(asOfDate: Date = new Date(), branchId?: string) {
    ...
    return {
      assets: accountBalances.filter(a => a.account_type === AccountType.asset),
      liabilities: accountBalances.filter(a => a.account_type === AccountType.liability),
      equity: accountBalances.filter(a => a.account_type === AccountType.equity),
      totalAssets: accountBalances.filter(a => a.account_type === AccountType.asset).reduce((sum, a) => sum + a.balance, 0),
      totalLiabilities: accountBalances.filter(a => a.account_type === AccountType.liability).reduce((sum, a) => sum + a.balance, 0),
      totalEquity: accountBalances.filter(a => a.account_type === AccountType.equity).reduce((sum, a) => sum + a.balance, 0),
    };
  }
  ...
  static async getIncomeStatement(startDate: Date, endDate: Date, branchId?: string) {
    ...
    const revenue = accountMovements.filter(a => a.account_type === AccountType.revenue).reduce((sum, a) => sum + a.amount, 0);
    const expenses = accountMovements.filter(a => a.account_type === AccountType.expense).reduce((sum, a) => sum + a.amount, 0);

    return {
      revenueItems: accountMovements.filter(a => a.account_type === AccountType.revenue),
      expenseItems: accountMovements.filter(a => a.account_type === AccountType.expense),
