...
export class BankService {
  ...
  async importStatement(
    ...
    const descIdx = header.findIndex(
      (h) => h.includes("desc") || h.includes("particulars"),
    );
    ...
    if (dateIdx === -1 || descIdx === -1 || amtIdx === -1) {
      throw new Error(
        "Invalid CSV Format. Required: Date, Description, Amount",
      );
    }
  ...
  async reconcileItems(bankTransactionId: string, systemTransactionId: string) {
    return await prisma.$transaction(async (tx) => {
      ...
      const systemTxn = await tx.bankTransaction.findUnique({
      ...
      const bankSigned =
        bankTxn.transaction_type === "expense"
          ? -bankTxn.amount
          : bankTxn.amount;
      const systemSigned =
        systemTxn.transaction_type === "expense"
          ? -systemTxn.amount
          : systemTxn.amount;

      if (Math.abs(bankSigned - systemSigned) > 0.01) {
        throw new Error(
          `Amount Mismatch: Statement ${bankSigned} vs System ${systemSigned}`,
        );
      }
