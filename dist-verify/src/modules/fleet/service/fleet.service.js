...
class FleetService {
    ...
    async getTrucks(query) {
        try {
            ...
            const data = trucks.map((truck) => this.formatTruckResponse(truck));
            const totalPages = Math.ceil(total / limit);
            logger_1.logger.info({ page, limit, total }, "Trucks fetched");
            return {
                data,
                pagination: {
                    page,
            ...
            logger_1.logger.error(error, "Failed to fetch trucks");
            throw error;
        }
    }
    /**
        ...
        try {
            ...
            const delivery = await this.prisma.$transaction(async (tx) => {
                ...
                if (sales.status !== "confirmed" && sales.status !== "shipped") {
                    throw new Error(`Sales order must be confirmed before delivery (current: ${sales.status})`);
                }
            ...
            return this.formatDeliveryDetailResponse(delivery);
        }
        catch (error) {
            logger_1.logger.error(error, "Failed to create delivery");
            throw error;
        }
    }
    /**
     ...
     */
    async updateDeliveryStatus(id, dto) {
        try {
            ...
            const delivery = await this.prisma.$transaction(async (tx) => {
                ...
                if (!allowedNext.includes(dto.status)) {
                    throw new Error(`Cannot transition from ${current.status} to ${dto.status}`);
                }
            ...
            logger_1.logger.info({ id, status: dto.status }, "Delivery status updated");
            return this.formatDeliveryDetailResponse(delivery);
        }
        catch (error) {
            logger_1.logger.error(error, "Failed to update delivery status");
            throw error;
        }
    }
    /**
    ...
    async getDeliveryTimeline(id) {
        try {
            ...
            if (!delivery) {
                throw (0, errors_1.notFoundError)("Delivery", id);
            }
            ...
            logger_1.logger.error(error, "Failed to fetch delivery timeline");
            throw error;
        }
    }
    /**
    ...
    async listDeliveries(query) {
        try {
            ...
            const data = deliveries.map((d) => this.formatDeliveryDetailResponse(d));
            const totalPages = Math.ceil(total / limit);
            return {
                data,
                pagination: {
            ...
            logger_1.logger.error(error, "Failed to list deliveries");
            throw error;
        }
    }
    ...
    formatTruckResponse(truck) {
        ...
        return {
            ...
            utilizationRate: truck.capacity > 0 ? (activeDeliveries / truck.capacity) * 100 : 0,
        };
    }
    formatDeliveryDetailResponse(delivery) {
        return {
            id: delivery.id,
            delivery_no: delivery.delivery_no,
            status: delivery.status,
    ...
    async createTruckLegacy(data) {
        ...
        logger_1.logger.info({ id: truck.id }, "Truck created (legacy)");
        return {
            id: truck.id,
            registration: truck.registration,
            model: truck.model,
     ...
     */
    async getTruckLegacy(id) {
        const truck = await this.prisma.truck.findUnique({
            where: { id },
        });
        if (!truck) {
            throw (0, errors_1.notFoundError)("Truck", id);
        }
        return {
            id: truck.id,
            registration: truck.registration,
     ...
     */
    async updateTruckLegacy(id, data) {
        ...
        logger_1.logger.info({ id }, "Truck updated (legacy)");
        return {
            id: updated.id,
     ...
     */
    async updateDeliveryLegacy(id, data) {
        ...
        logger_1.logger.info({ id, status: updated.status }, "Delivery updated (legacy)");
        return {
            id: updated.id,
