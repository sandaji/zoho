...
class BranchService {
    ...
    async getBranchDashboard(branchId) {
        try {
            logger_1.logger.info({ branchId }, "Fetching branch dashboard");
            ...
            if (!branch) {
                throw (0, errors_1.notFoundError)(`Branch not found: ${branchId}`);
            }
            ...
            const [kpis, salesMetrics, operationsMetrics, payrollMetrics, recentSales, recentDeliveries, recentTransactions, alerts,] = await Promise.all([
            ...
            logger_1.logger.info({ branchId }, "Branch dashboard fetched successfully");
            return dashboard;
        }
        catch (error) {
            logger_1.logger.error({ error }, "Error fetching branch dashboard");
            throw error;
        }
    }
    /**
    ...
    async getAllBranchSummaries() {
        try {
            ...
            logger_1.logger.error({ error }, "Error fetching branch summaries");
            throw error;
        }
    }
    /**
     ...
     */
    async getBranchKPIs(branchId, monthStart, monthEnd) {
        try {
            const branch = await this.prisma.branch.findUnique({
                where: { id: branchId },
            });
            ...
            const monthSales = await this.prisma.sales.findMany({
                where: {
                    branchId,
                    createdAt: {
                        gte: monthStart,
            ...
            const totalRevenue = allSales.reduce((sum, s) => sum + (s.grand_total || 0), 0);
            ...
            const lowStockItems = inventoryData.filter((inv) => (inv.quantity || 0) < 10).length;
            // Fleet
            ...
            const activeDeliveries = deliveries.filter((d) => d.status === "pending" || d.status === "in_transit").length;
            ...
            const deliverySuccessRate = deliveries.length > 0
                ...
                : 0;
            // Financial metrics
            const expenses = await this.prisma.financeTransaction.findMany({
                where: {
                    type: "expense",
                    createdAt: {
                        gte: monthStart,
            ...
            return {
                ...
                total_revenue: totalRevenue,
                average_order_value: allSales.length > 0 ? totalRevenue / allSales.length : 0,
            ...
            logger_1.logger.error({ error }, "Error calculating branch KPIs");
            throw error;
        }
    }
    /**
     ...
     */
    async getBranchSalesMetrics(branchId, monthStart, monthEnd) {
        try {
            ...
            logger_1.logger.error({ error }, "Error calculating sales metrics");
            throw error;
        }
    }
    /**
     ...
     */
    async getBranchOperationsMetrics(branchId) {
        try {
            // Staff
            const employees = await this.prisma.user.findMany({
                where: { branchId },
            ...
            const lowStockItems = inventory.filter((inv) => (inv.quantity || 0) < 10).length;
            const outOfStockItems = inventory.filter((inv) => (inv.quantity || 0) === 0).length;
            const totalItems = inventory.reduce((sum, inv) => sum + (inv.quantity || 0), 0);
            const inventoryTurnover = totalItems > 0 ? inventory.length / totalItems : 0;
            // Fleet
            const trucks = await this.prisma.truck.findMany({
                where: { deliveries: { some: { sales: { branchId } } } },
            });
            const activeTrucks = trucks.filter((t) => t.isActive).length;
            const maintenancePending = trucks.filter((t) => !t.isActive).length;
            const fuelCost = 0; // fuel_cost not in schema
            const utilizationRate = trucks.length > 0 ? (activeTrucks / trucks.length) * 100 : 0;
            // Warehouses
            ...
            const warehouseCapacity = warehouses.map((w) => ({
                ...
                current_items: inventory.filter((inv) => inv.warehouseId === w.id).length,
                utilization_percentage: (w.capacity || 1) > 0
                    ? (inventory.filter((inv) => inv.warehouseId === w.id)
                        .length /
                        (w.capacity || 1)) *
                        100
                    : 0,
            ...
            return {
                ...
                total_employees: employees.length,
                active_employees: activeEmployees,
                employee_attendance_rate: employees.length > 0 ? (activeEmployees / employees.length) * 100 : 0,
            ...
            logger_1.logger.error({ error }, "Error calculating operations metrics");
            throw error;
        }
    }
    /**
     ...
     */
    async getBranchPayrollMetrics(branchId, month, year) {
        try {
            ...
            const branchPayrolls = payrolls.filter((p) => p.user?.branchId === branchId);
            const totalCost = branchPayrolls.reduce((sum, p) => sum + (p.net_salary || 0), 0);
            const totalAllowances = branchPayrolls.reduce((sum, p) => sum + (p.allowances || 0), 0);
            const totalDeductions = branchPayrolls.reduce((sum, p) => sum + (p.deductions || 0), 0);
            const totalTaxes = totalCost * 0.1;
            const paidCount = branchPayrolls.filter((p) => p.status === "paid").length;
            const pendingCount = branchPayrolls.filter((p) => p.status === "pending").length;
            const failedCount = branchPayrolls.filter((p) => p.status === "failed").length;
            const salaries = branchPayrolls.map((p) => p.net_salary || 0);
            const avgSalary = branchPayrolls.length > 0 ? totalCost / branchPayrolls.length : 0;
            const maxSalary = salaries.length > 0 ? Math.max(...salaries) : 0;
            const minSalary = salaries.length > 0
                ? Math.min(...salaries.filter((s) => s > 0))
                : 0;
            const departmentMap = new Map();
            branchPayrolls.forEach((payroll) => {
            ...
            return {
                ...
                active_payroll_runs: branchPayrolls.filter((p) => p.status === "processed" || p.status === "paid").length,
            ...
            logger_1.logger.error({ error }, "Error calculating payroll metrics");
            throw error;
        }
    }
    /**
     ...
     */
    async getRecentSales(branchId, limit) {
        const sales = await this.prisma.sales.findMany({
            where: { branchId },
     ...
     */
    async getRecentDeliveries(branchId, limit) {
        const deliveries = await this.prisma.delivery.findMany({
     ...
     */
    async getRecentTransactions(branchId, limit) {
    ...
    async generateAlerts(branchId) {
        ...
        try {
            ...
            logger_1.logger.error({ error }, "Error generating alerts");
        }
        return alerts;
    }
    // ============================================================================
    // HELPER METHODS
    // ============================================================================
    async getTopProducts(branchId, limit) {
        const now = new Date();
        const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
    ...
    calculateDailySalesTrend(sales) {
    ...
    async getTopCustomers(branchId, limit) {
        const now = new Date();
        const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
    ...
    calculateAverageDeliveryTime(deliveries) {
        const completedDeliveries = deliveries.filter((d) => d.status === "delivered" && d.expected_delivery);
        if (completedDeliveries.length === 0)
            return 0;
        const totalTime = completedDeliveries.reduce((sum, d) => {
            const createdTime = new Date(d.createdAt).getTime();
            const deliveredTime = new Date(d.expected_delivery || new Date()).getTime();
            return sum + (deliveredTime - createdTime);
        }, 0);
        return totalTime / (completedDeliveries.length * 3600000);
    }
    async getLastMonthRevenue(branchId) {
        const now = new Date();
        const lastMonthStart = new Date(now.getFullYear(), now.getMonth() - 1, 1);
        const lastMonthEnd = new Date(now.getFullYear(), now.getMonth(), 0);
        const lastMonthSales = await this.prisma.sales.findMany({
            where: {
                branchId,
                createdAt: {
                    gte: lastMonthStart,
        ...
        return lastMonthSales.reduce((sum, s) => sum + (s.grand_total || 0), 0);
    }
}
