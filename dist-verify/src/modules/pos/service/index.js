...
class PosService {
    ...
    async searchProduct(dto) {
        ...
        if (!products || products.length === 0) {
            throw new errors_1.AppError(errors_1.ErrorCode.NOT_FOUND, 404, `No products found matching: ${searchTerm}`);
        }
        ...
        const results = await Promise.all(products.map(async (product) => {
            ...
            if (warehouseIds.length > 0) {
                ...
                if (inventory.length > 0) {
                    totalAvailable = inventory.reduce((sum, inv) => sum + (inv.available || 0), 0);
    ...
    async createSales(dto) {
        const { branchId, userId, items, payment_method, amount_paid, discount = 0, discount_approved_by, notes, } = dto;
        // Basic validations
        ...
        if (discountPercent > 10 && !discount_approved_by) {
            throw new errors_1.AppError(errors_1.ErrorCode.FORBIDDEN, 403, "Discounts over 10% require manager approval");
        }
        ...
        return await this.prisma.$transaction(async (tx) => {
            ...
            for (const item of items) {
                ...
                if (!product) {
                    throw new errors_1.AppError(errors_1.ErrorCode.NOT_FOUND, 404, `Product not found: ${item.productId}`);
                }
                ...
                if (inventory) {
                    ...
                    if (!product.quantity || product.quantity < item.quantity) {
                        throw new errors_1.AppError(errors_1.ErrorCode.VALIDATION_ERROR, 400, `Insufficient inventory for product: ${product.name} in branch ${branch.name}`);
                    }
                }
    ...
    async getSalesById(id) {
        ...
        if (!sale)
            throw new errors_1.AppError(errors_1.ErrorCode.NOT_FOUND, 404, "Sale not found");
    ...
    async listSales(query) {
        const { page = 1, limit = 20, status, branchId, startDate, endDate, payment_method, } = query;
        const where = {};
        if (status)
            where.status = status;
        if (branchId)
        ...
        if (startDate || endDate) {
            where.created_date = {};
            if (startDate)
                where.created_date.gte = new Date(startDate);
            if (endDate)
                where.created_date.lte = new Date(endDate);
        }
        ...
        return {
            data: sales.map((s) => this.formatSalesResponse(s, s.items)),
            total,
        };
    }
    /**
    ...
    async getDailySummary(dto) {
        ...
        const total_tax = sales.reduce((sum, s) => sum + s.tax, 0);
        ...
        for (const s of sales) {
            ...
            if (pm in paymentsAgg)
                paymentsAgg[pm] += s.grand_total;
        }
     ...
     * Approve discount
     */
    async approveDiscount(dto) {
        const { salesId, managerId, managerPassword } = dto;
        const manager = await this.prisma.user.findUnique({
            where: { id: managerId },
        });
        if (!manager)
            throw new errors_1.AppError(errors_1.ErrorCode.NOT_FOUND, 404, "Manager not found");
        if (!["manager", "admin"].includes(manager.role)) {
            throw new errors_1.AppError(errors_1.ErrorCode.FORBIDDEN, 403, "Only managers or admins can approve discounts");
        }
        const isValid = await (0, password_1.verifyPassword)(managerPassword, manager.passwordHash);
        if (!isValid)
            throw new errors_1.AppError(errors_1.ErrorCode.UNAUTHORIZED, 401, "Invalid manager password");
        await this.prisma.sales.update({
            where: { id: salesId },
            data: { discount_approved_by: managerId },
        });
        logger_1.logger.info({ salesId, managerId }, "Discount approved");
    }
    // ============================================================================
    // PRIVATE HELPERS
    // ============================================================================
    ...
    formatSalesResponse(sale, items) {
        return {
            id: sale.id,
