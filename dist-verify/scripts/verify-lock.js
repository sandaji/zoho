...
const period_service_1 = require("../src/modules/finance/services/period.service");
...
async function main() {
    ...
    if (!period) {
        ...
        await period_service_1.PeriodService.initializeFiscalYear(new Date().getFullYear());
    ...
    await period_service_1.PeriodService.lock(period.id, user.id);
    ...
    if (lockLog && lockLog.changes.action === 'LOCK') {
    ...
    await period_service_1.PeriodService.unlock(period.id, user.id);
    ...
    if (unlockLog && unlockLog.changes.action === 'UNLOCK') {
