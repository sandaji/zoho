...
const bank_service_1 = require("../src/modules/finance/services/bank.service");
...
async function main() {
    ...
    const bankService = new bank_service_1.BankService();
    ...
    try {
        ...
        if (accounts.length === 0) {
            ...
            const acc = await db_1.prisma.chartOfAccount.create({
                data: {
        ...
        const je = await db_1.prisma.journalEntry.create({
            data: {
                entry_no: `JE-${testId}`,
        ...
        if (!matchedLine)
            throw new Error("Import failed or Line not found");
        ...
        if (!matchedJe)
            throw new Error("Journal Entry not found in reconciliation list");
        ...
        // 6. Verify result
        const checkLine = await db_1.prisma.bankStatementLine.findUnique({ where: { id: bankLineId } });
        const checkJe = await db_1.prisma.journalEntry.findUnique({ where: { id: journalEntryId } });
        ...
        if (bankLineId) {
            ...
            // Find statement
            const line = await db_1.prisma.bankStatementLine.findUnique({ where: { id: bankLineId } });
            if (line) {
                await db_1.prisma.bankStatement.delete({ where: { id: line.statement_id } });
            }
        }
        if (journalEntryId) {
            await db_1.prisma.journalEntry.delete({ where: { id: journalEntryId } });
        }
        if (accountId && accountId.includes(testId)) { // Only delete if we created it
            await db_1.prisma.chartOfAccount.delete({ where: { id: accountId } });
        }
        console.log('Done.');
    }
}
