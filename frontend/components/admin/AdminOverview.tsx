    ...
    <div className="space-y-6 pb-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-emerald-900">Global Command Center</h2>
            {lastUpdated && (
      ...
      <Card className="rounded-xl border border-emerald-100 bg-white shadow-sm">
        <CardHeader className="flex flex-row items-center justify-between pb-3">
          <CardTitle className="text-sm text-emerald-900">Today's Top Products</CardTitle>
                ...
                {safe.top_products.slice(0, 8).map((product, index) => {
                  ...
                  return (
                    <TableRow
                      ...
                      className="group border-emerald-50 hover:bg-emerald-50/50"
                    >
                      <TableCell>
                        <div className="flex items-center gap-3">
      ...
      {/* ── Global Financials Widget ────────────────────────────────────── */}
      {permissions.canViewFinancials && <GlobalFinancialsWidget />}

      {/* ── Multi-Branch Widgets Grid ───────────────────────────────────── */}
      <div className="grid gap-6 lg:grid-cols-2">
        {/* IBT Monitor */}
        {permissions.canViewIBT && <IBTMonitorWidget />}

        {/* System Health */}
        {permissions.canViewSystemHealth && <SystemHealthWidget />}
      </div>

      {/* ── Global Audit Feed ───────────────────────────────────────────── */}
      {permissions.canViewAuditFeed && <GlobalAuditFeed />}
    </div>
  );
}
